        ...
        for(int f = 0; f < out_depth; f++) {
            ...
            for(int out_y = 0; out_y < out_height; y += stride, out_y++) {
                ...
                for(int out_x = 0; out_x < out_width; x += stride, out_x++) {
                    ...
                    double sum = 0.0;
                    __m256d vector_val = _mm256_setzero_pd();
                    for(int fy = 0; fy < f_height; fy++) {
                        ...
                        for(int fx = 0; fx < filter->width; fx++) {
                            ...
                            if(in_y >= 0 && in_y < in->height && in_x >=0 && in_x < in->width) {
                                ...
                                int inside = ((filter_width * fy) + fx) * filter_depth;
                                int in_inside = ((in_width * in_y) + in_x) * in_depth;
                                if (filter_depth == 3) {
                                    //filter_depth = 0;
                                    sum+= filter->weights[inside] * in->weights[in_inside];
                                    sum+= filter->weights[inside + 1] * in->weights[in_inside + 1];
                                    sum+= filter->weights[inside + 2] * in->weights[in_inside + 2];
                                } else if (filter_depth == 16){


                                    __m256d load1 = _mm256_loadu_pd(&filter->weights[inside]);
                                    __m256d load2 = _mm256_loadu_pd(&in->weights[in_inside]);
                                    vector_val = _mm256_add_pd(vector_val, _mm256_mul_pd(load1, load2));

                                    load1 = _mm256_loadu_pd(&filter->weights[inside + 4]);
                                    load2 = _mm256_loadu_pd(&in->weights[in_inside + 4]);
                                    vector_val = _mm256_add_pd(vector_val, _mm256_mul_pd(load1, load2));



                                    load1 = _mm256_loadu_pd(&filter->weights[inside + 8]);
                                    load2 = _mm256_loadu_pd(&in->weights[in_inside + 8]);
                                    vector_val = _mm256_add_pd(vector_val, _mm256_mul_pd(load1, load2));

                                    load1 = _mm256_loadu_pd(&filter->weights[inside + 12]);
                                    load2 = _mm256_loadu_pd(&in->weights[in_inside + 12]);
                                    vector_val = _mm256_add_pd(vector_val, _mm256_mul_pd(load1, load2));
                                    ...
                                    load1 = _mm256_loadu_pd(&filter->weights[inside + 16]);
                                    load2 = _mm256_loadu_pd(&in->weights[in_inside + 16]);
                                    vector_val = _mm256_add_pd(vector_val, _mm256_mul_pd(load1, load2));
                                }
        ...
        for (int x = 0; x < l->input_width; x++) {
            for (int y = 0; y < l->input_height; y++) {
                for (int d = 0; d < l->input_depth; d++) {
                    double det = inputs[i]->weights[((inputs[i]->width * y) + x) * inputs[i]->depth + d];
                    double value = (det < 0.0) ? 0.0 : det;
